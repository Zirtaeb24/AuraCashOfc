    ...
    renderMetas(metas) {
        this.list.innerHTML = metas.map(meta => `
            <div class="card" style="margin: 10px 0; padding: 15px;">
                <h4>${meta.categoryName}</h4>
                <p><strong>Meta:</strong> ${Utils.formatCurrency(meta.amount)}</p>
                <p><strong>Período:</strong> ${Utils.formatDate(meta.from)} até ${Utils.formatDate(meta.to)}</p>
                <div class="progress">
